...
float p_temp, i_temp, d_temp;//给到freertos中进行调用，打印出相关值
uint8_t pid_flag = 0;
...
int8_t receiving_process(void)
{
  ...
  uint16_t cmd_type = CMD_NONE;     // 命令类型
...
//        float p_temp, i_temp, d_temp;  
	    ...
	    pid_flag = 1;//数据发下的标志位置1
//		printf("这个时候的目标值:%2.f %2.f %2.f \r\n",p_temp,i_temp,d_temp);
      }
	  break;
      ...
      case SET_TARGET_CMD:
      {
		...
		pid.target_val = set_point;   // 这一步必不可少，野火里面少了这一步。
	    target_flag = 1;
//		printf("这个时候的目标值:%2.f\r\n",pid.target_val);
      }
      break;
...
//		  TIM_SetTIM4Compare1(0);
      ...
      case SET_PERIOD_CMD:
      {
        ...
        temp_hz = COMPOUND_32BIT(&frame_data[13]);     // 周期数
        SET_BASIC_TIM_PERIOD(temp_hz);                             // 设置定时器周期1~1000ms
	    hz_flag = 1;
//		printf("周期数指令是：%d",temp);
      }
      break;
...
/**
  ...
  * @retval 无
  */
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)//这里的data的变量类型进行了改动，注意
{
  uint8_t sum = 0;    // 校验和
  num *= 4;           // 一个参数 4 个字节
  ...
  HAL_UART_Transmit(&huart3, (uint8_t *)&set_packet, sizeof(set_packet), 0xFFFFF);    // 发送数据头
  HAL_UART_Transmit(&huart3, (uint8_t *)data, num, 0xFFFFF);                          // 发送参数
  HAL_UART_Transmit(&huart3, (uint8_t *)&sum, sizeof(sum), 0xFFFFF);                  // 发送校验和
}
